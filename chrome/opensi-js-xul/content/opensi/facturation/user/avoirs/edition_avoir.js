...
var aLignes = new Arbre('Facturation/GetRDF/articles_avoir.tmpl', 'articles');
...
function init2() {
  try {
		...
		if (mode=="C") {
			...
			avoir_id = ParamValeur("Avoir_Id");

			initLigne = true;

			aLignes.setParam('Avoir_Id', avoir_id);
			aLignes.initTree(chargerAvoir);
		}
...
function validerLigne() {
  try {
		...
		if (ok) {
			...
			if (mode_ligne=="C") {
				var corps = cookie() +"&Page=Facturation/Avoirs/ajouterArticle.tmpl&ContentType=xml";
			}
			else {
				var corps = cookie() +"&Page=Facturation/Avoirs/modifierArticle.tmpl&ContentType=xml";
...
function supprimerLigne() {
  try {
		...
		var corps = cookie() +"&Page=Facturation/Avoirs/supprimerArticle.tmpl&ContentType=xml";
    corps += "&Avoir_Id="+ avoir_id +"&Ligne_Id="+ ligneId;
		requeteHTTP(corps);
...
function enregistrerAvoir() {
  try {
    ...
    var remiseFP = document.getElementById('RemiseFP').value;
		...
		var modeEnvoiAvoir = document.getElementById('modeEnvoiAvoir').value;
    ...
    var denomination_fact = document.getElementById('Denomination_Fact').value;
		...
		if (isEmpty(remise) || (type_remise=='P'?!isTaux(remise):!isPositiveOrNull(remise) || parseFloat(remise)>montant_base)) { showWarning("Remise incorrecte !"); }
			...
			if (mode=="C") {
				corps = cookie() +"&Page=Facturation/Avoirs/creerAvoir.tmpl&ContentType=xml";
			}
			else {
				corps = cookie() +"&Page=Facturation/Avoirs/modifierAvoir.tmpl&ContentType=xml&Avoir_Id="+ avoir_id;
			}
			...
			corps += "&Util_R="+ urlEncode(document.getElementById('Login_Resp').value);
			corps += "&Mode_Envoi_Avoir="+ urlEncode(modeEnvoiAvoir);
      ...
      corps += "&Escompte="+ escompte;
			...
			if (mode=="C") {
				avoir_id = p.responseXML.documentElement.getAttribute("Avoir_Id");
				aLignes.setParam('Avoir_Id', avoir_id);
				charger=false;
				bloquerInterface();
				chargerAvoir();
...
function setModifie(m) {
  try {
		...
		if (m) {
			...
			document.getElementById('tabAvoir').setAttribute('image', 'chrome://opensi/content/design/icones/modified.png');
			...
			document.getElementById('tabAvoir').setAttribute('image', null);
...
function debloquerInterface() {
	try {
		...
		document.getElementById('modeEnvoiAvoir').disabled=false;
...
function nouvelAvoir() {
  try {
		...
		document.getElementById('modeEnvoiAvoir').value = result.responseXML.documentElement.getAttribute('Def_Mode_Envoi_Facture');
		
		document.getElementById('Commentaires_Fin').value = "";
		document.getElementById('Commentaires_Int').value = "";
		document.getElementById('Date_Avoir').value = "";
...
function chargerAvoir() {
  try {

		if (mode != 'V') mode = "M";
		var corps = cookie() +"&Page=Facturation/Avoirs/getAvoir.tmpl&ContentType=xml&Avoir_Id="+ avoir_id;
		var p = requeteHTTP(corps);
		...
		document.getElementById('modeEnvoiAvoir').value = contenu.getAttribute('Mode_Envoi_Avoir');
    ...
    document.getElementById('Frais_Port').value = contenu.getAttribute('Frais_Port');
		...
		document.getElementById('Date_Avoir').value = contenu.getAttribute('Date_Avoir');
    ...
    assujettiTVA = (contenu.getAttribute('Assujetti_TVA')=="1");
		...
		document.getElementById('Creation').label = "Avoir créé le "+ contenu.getAttribute('Date_Creation') + " par "+ contenu.getAttribute('Login_Createur');
		...
		document.getElementById('Fiche').label = "Avoir N° "+ contenu.getAttribute('Num_Entier');
    ...
    initVersion();
		...
		if (mode!="V") {
			...
			document.getElementById('bSupAvoir').disabled=false;
			document.getElementById('Stock').collapsed = false;
			debloquerInterface();
		} else {
			document.getElementById('listeVersion').disabled=false;
			document.getElementById('articles').disabled=false;
...
function visualiserAvoir() {
  try {

		var tree = document.getElementById("articles");
    var frais_port = document.getElementById('Frais_Port').value;

		if ((tree.view==null || tree.view.rowCount<=0) && frais_port==0) {
			showWarning("L'avoir ne contient aucune ligne !");
		}
		else if (gen || window.confirm("Confirmez-vous la génération de l'avoir ?\n(Attention l'avoir généré ne pourra plus être modifiée !)")) {

			var page = "chrome://opensi/content/facturation/user/avoirs/avoir.xul?"+ cookie();
      page += "&Avoir_Id="+ avoir_id;
      page += "&Mode=V";
      page += "&Stock="+document.getElementById('Stock').checked;
...
function apercuAvoir() {
  try {
    ...
    var frais_port = document.getElementById('Frais_Port').value;
		...
		if ((tree.view==null || tree.view.rowCount<=0) && frais_port==0) { showWarning("L'avoir ne contient aucune ligne !"); }
...
function supprimerAvoir() {
  try {

		if (window.confirm("Confirmez-vous la suppression de l'avoir ?")) {

			var corps = cookie() +"&Page=Facturation/Avoirs/supprimerAvoir.tmpl&ContentType=xml&Avoir_Id="+ avoir_id;
			requeteHTTP(corps);
...
function editerCommentaire() {
  try {
		...
		if (tree.view!=null && tree.currentIndex!=-1) {
			...
			var url = "chrome://opensi/content/facturation/user/avoirs/commentaire.xul?"+ cookie();
...
function editerCommentairesCaches() {
  try {
		...
		var url = "chrome://opensi/content/facturation/user/avoirs/commentaire_cache.xul?"+ cookie();
    url += "&Avoir_Id="+ avoir_id;
  	window.openDialog(url,'','chrome,modal,centerscreen');
...
function choisirMentions() {
  try {
  	...
  	if (mode=="C") {
		...
		if (ok) {
			var url = "chrome://opensi/content/facturation/user/commun/popup-choix_mentions.xul?"+ cookie() +"&Type_Doc=Avoir&Doc_Id="+ avoir_id;
...
function initVersion() {
	try {
		...
		aVersion.setParam("Type_Document", "Avoir");
		aVersion.setParam("Document_Id", avoir_id);
		aVersion.initTree();
...
function retour_avoirs() {
  try {

    window.location = "chrome://opensi/content/facturation/user/avoirs/menu_avoirs.xul?"+ cookie();
