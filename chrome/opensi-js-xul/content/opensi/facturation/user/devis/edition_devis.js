...
function demandeEnregistrement() {
  try {
		...
		if (modifie) {
			if (window.confirm("Voulez-vous enregistrer les modifications apportées au devis ?")) {
				enregistrerDevis();
			}
		}
...
function chargerDevis() {
  try {
    ...
    document.getElementById('Client_Id').value = client_id;
		...
		document.getElementById('Creation').label = "Devis créé le "+ contenu.getAttribute('Date_Creation') +" par "+ contenu.getAttribute('Login_Createur');
...
function archiverDevis() {
	try {
		if (window.confirm("Etes-vous sûr de vouloir archiver le devis ?")) {
