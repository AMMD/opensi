...
function init() {
	try {
		...
		if (typeDoc=="Devis") {
			...
			sansTotaux = (ParamValeur("SansTotaux")=="true");
			var libelleSujet = "Devis N° ";
...
function envoyer() {
	try {
		...
		if (!isEmail(expediteur)) { showWarning("L'adresse e-mail de l'expéditeur est invalide !"); }
			...
			var qEnvoiMail;
			if (typeDoc=="Devis") {
				qEnvoiMail = new QueryHttp("Facturation/Devis/devis_pdf.tmpl");
				qEnvoiMail.setParam("Devis_Id", docId);
				if (sansTotaux) {
					qEnvoiMail.setParam("SansTotaux", "1");
				}
			} else if (typeDoc=="Proforma") {
			...
			var message = "";
			if (typeDoc=="Devis") {
				message = "Le devis a été envoyé par e-mail !";
			} else if (typeDoc=="Proforma") {
