...
var aListeEcheances = new Arbre('Facturation/Factu_Directe/liste-echeancesMultiples.tmpl', 'listeEcheances');
...
function init() {
	try {
		...
		var qGetMontantNet = new QueryHttp("Facturation/Factu_Directe/getMontantNet.tmpl");
...
function validerLigne() {
	try {
		...
		if (isEmpty(dateEcheance) || !isDate(dateEcheance)) { showWarning("Date d'échéance incorrecte !"); }
			...
			if (currentEcheance=="") {
				qEnregistrerEcheance = new QueryHttp("Facturation/Factu_Directe/creerEcheance.tmpl");
				qEnregistrerEcheance.setParam("Facture_Id", factureId);
			} else {
				qEnregistrerEcheance = new QueryHttp("Facturation/Factu_Directe/modifierEcheance.tmpl");
...
function supprimerLigne() {
	try {
		if (window.confirm("Voulez-vous supprimer cette échéance ?")) {
			var qSupprimerEcheance = new QueryHttp("Facturation/Factu_Directe/supprimerEcheance.tmpl");
...
function reinitialiser() {
	try {
		if (window.confirm("Voulez-vous réinitialiser les échéances ?")) {
			var qReinitialiser = new QueryHttp("Facturation/Factu_Directe/reinitialiserEcheances.tmpl");
