...
function valider() {
	try {
		...
		if (isEmpty(nouvelArticle)) { showWarning("Veuillez saisir une référence pour le nouvel article !"); }
		else if (!isCleAlpha(nouvelArticle)) { showWarning("Référence article invalide !"); }
		else {
			...
			if (result.responseXML.documentElement.getAttribute("existe") == "true") {
				showWarning("Cette référence est déjà utilisée !");
			}
			else {
