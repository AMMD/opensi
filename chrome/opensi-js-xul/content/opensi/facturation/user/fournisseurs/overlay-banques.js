...
var currentBanque;
var aBanques = new Arbre("Facturation/GetRDF/banques_fournisseur.tmpl", "tree-banques");


function initBanques() {
	try {

		aBanques.setParam("Fournisseur_Id", currentFournisseur);
		aBanques.initTree();
		nouveauBanque();
...
function supprimerBanque() {
	try {
		...
		if (window.confirm("Confirmez-vous la suppression de la banque '"+ nom +"' ?")) {
			var corps = cookie() +"&Page=Facturation/Fournisseurs/supprimerBanqueFournisseur.tmpl&ContentType=xml&Fournisseur_Id="+ currentFournisseur +"&Banque_Id="+ currentBanque;
...
function enregistrerBanque(mode) {
    ...
    var international = document.getElementById('international').checked;
		...
		if (mode=="C") {
			corps = cookie() +"&Page=Facturation/Fournisseurs/creerBanqueFournisseur.tmpl&ContentType=xml";
		}
		else {
			corps = cookie() +"&Page=Facturation/Fournisseurs/modifierBanqueFournisseur.tmpl&ContentType=xml&Banque_Id="+ currentBanque;
		}

    corps += "&Fournisseur_Id="+ urlEncode(currentFournisseur) +"&Nom="+ urlEncode(nom) +"&Adresse="+ urlEncode(adresse) + "&BIC="+ bic;
    if (!international) {
...
function chargerBanque() {
	try {
		...
		if (tree.view!=null && tree.currentIndex!=-1) {
			...
			var corps = cookie() +"&Page=Facturation/Fournisseurs/getBanqueFournisseur.tmpl&ContentType=xml&Banque_Id="+ banque_id +"&Fournisseur_Id="+ currentFournisseur;
