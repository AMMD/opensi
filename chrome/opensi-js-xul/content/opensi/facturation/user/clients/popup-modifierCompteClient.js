...
function init() {
  try {
  	...
  	var qClient = new QueryHttp("Facturation/Clients/getNumeroCompte.tmpl");
  	qClient.setParam("Client_Id", clientId);
		...
		document.getElementById('lblClientId').value = clientId;
...
function enregistrer() {
	try {
		...
		if (isEmpty(numCompte)) { showWarning("Veuillez choisir un numéro de compte !"); }
			...
			var qAffecterCompte = new QueryHttp("Facturation/Clients/modifierNumeroCompte.tmpl");
			qAffecterCompte.setParam("Client_Id", clientId);
