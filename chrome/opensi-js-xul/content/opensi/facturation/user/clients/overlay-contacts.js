...
var currentContact;
var aContacts = new Arbre("Facturation/GetRDF/liste-contactsClient.tmpl", "tree-contacts");
...
function initContacts() {
  try {
		...
		aPays.initTree(initPaysContact);

		aContacts.setParam("Client_Id", currentClient);
...
function nouveauContact() {
	try {
	  ...
	  document.getElementById('oc-Civilite').value = "1";
		...
		document.getElementById('oc-Principal_Fact').checked = false;
		document.getElementById('oc-Principal_Liv').checked = false;
		document.getElementById('oc-Principal_Envoi').checked = false;
...
function pressOnTreeContacts(ev) {
	try {
...
function chargerContact() {
	try {
		...
		if (aContacts.isSelected()) {
			...
			document.getElementById('oc-Principal_Fact').checked = (aContacts.getSelectedCellText('oc-ColPrincipal_Fact')=="1");
			document.getElementById('oc-Principal_Liv').checked = (aContacts.getSelectedCellText('oc-ColPrincipal_Liv')=="1");
			document.getElementById('oc-Principal_Envoi').checked = (aContacts.getSelectedCellText('oc-ColPrincipal_Envoi')=="1");
...
function supprimerContact() {
	try {
		...
		if (window.confirm("Confirmez-vous la suppression du contact '"+ prenom +" "+ nom +"' ?")) {
			var corps = cookie() +"&Page=Facturation/Clients/supprimerContact.tmpl&ContentType=xml&Num_Inter="+ currentContact;
...
function enregistrerContact(mode) {
    ...
    var civilite = document.getElementById('oc-Civilite').value;
		...
		var principal_fact = document.getElementById('oc-Principal_Fact').checked?"1":"0";
		var principal_liv = document.getElementById('oc-Principal_Liv').checked?"1":"0";
		var principal_envoi = document.getElementById('oc-Principal_Envoi').checked?"1":"0";
		var dateNaissance = document.getElementById('oc-dateNaissance').value;

		var corps;

		if (mode=="C") {
			corps = cookie() +"&Page=Facturation/Clients/creerContact.tmpl&ContentType=xml";
		}
		else {
			corps = cookie() +"&Page=Facturation/Clients/modifierContact.tmpl&ContentType=xml&Num_Inter="+ currentContact;
		}

		corps += "&Client_Id="+ urlEncode(currentClient) +"&Nom="+ urlEncode(nom) +"&Prenom="+ urlEncode(prenom) +"&Fonction="+ urlEncode(fonction);
		corps += "&Relation="+ relation +"&Infos="+ urlEncode(infos) +"&Principal_Fact="+ principal_fact +"&Principal_Liv="+ principal_liv +"&Principal_Envoi="+ principal_envoi;
		...
		if (isEmpty(nom)) {
			...
			if (principal_fact==1) {
				defaut_contact_fact = currentContact;
			} else if (defaut_contact_fact==currentContact) {
				defaut_contact_fact = "";
			}
			if (principal_liv==1) {
				defaut_contact_liv = currentContact;
			} else if (defaut_contact_liv==currentContact) {
				defaut_contact_liv = "";
			}
			if (principal_envoi==1) {
				defaut_contact_envoi = currentContact;
			} else if (defaut_contact_envoi==currentContact) {
