...
function enregistrementArticles()
{
	try
	{
		...
		var page = cookie()+"&Page=WebManager/enregistrerArticlesTemp.tmpl&ContentType=xml";
...
function requeteTerminee(requete)
{
	try
	{
		...
		if(reussite=="OUI")
		{
			...
			document.getElementById('info').value="L'enregistrement a réussi.";
		...
		else if(reussite=="NON")
		{
			...
			document.getElementById('info').value="L'enregistrement a échouée.";
		}
...
function etape_suivante()
{
	try
	{
		window.location = "chrome://opensi/content/web_manager/attrib_caracs_web/selection_web_articles.xul?"+ cookie()
				+"&site_id="+site_id+"&nom_site="+nom_site+"&source=INIT";
	}
	catch (e)
	{
		recup_erreur(e);
...
function etape_precedente()
{
	try
	{
		window.location = "chrome://opensi/content/web_manager/initVL/gestionDoublons.xul?"+ cookie()
