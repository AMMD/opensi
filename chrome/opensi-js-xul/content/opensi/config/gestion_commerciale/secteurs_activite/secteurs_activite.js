...
function supprimerSecteur() {
	try {
		...
		if (tree.view!=null && tree.currentIndex!=-1) {
			...
			if (window.confirm("Confirmez-vous la suppression du secteur d'activité '"+ secteur +"' ?")) {
				...
				if (result.responseXML.documentElement.getAttribute("ok")=="false") {
					showWarning("Impossible de supprimer ce secteur d'activité car il est utilisé !");
				} else {
					aSecteurs.initTree();
					nouveauSecteur();
				}
	...
	try {
		...
		if (isEmpty(secteur)) {
			...
			if (rep.responseXML.documentElement.getAttribute('existe')=="true") {
				showWarning("Le secteur d'activité '"+ secteur +"' ou le code secteur '"+ codeSecteur +"' existe déjà !");
			}
			else {
				aSecteurs.initTree();
				nouveauSecteur();
