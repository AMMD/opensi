	...
	try {
		...
		if (window.confirm("Confirmez-vous la suppression de la banque '"+ nom +"' ?")) {
			...
			if (result.responseXML.documentElement.getAttribute("ok")=="true") {
				...
				showWarning("Impossible de supprimer la banque car elle est utilisée !");
			}
		}
...
function enregistrerBanque(mode) {
	try {
		...
		else if (!international && !isEmpty(deb_iban) && !verifCleIBAN(deb_iban.substring(2,4), deb_iban, fin_iban)) { showWarning("Echec du contrôle de la clé IBAN ! Erreur probable sur Numéro IBAN, Numéro de compte, Code agence ou Code guichet"); }
		
		else if (international && (isEmpty(deb_iban) || !isPrefIBAN(deb_iban) || isEmpty(fin_iban))) { showWarning("IBAN incorrect !"); }
		else if (international && !verifCleIBAN(deb_iban.substring(2,4), deb_iban, fin_iban)) { showWarning("Echec du contrôle de la clé IBAN !"); }
