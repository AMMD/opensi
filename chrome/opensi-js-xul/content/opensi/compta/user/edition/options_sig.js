...
function cacheChampsMail(b) {
	try {
		...
		if (b) {
			document.getElementById('BoutonEdition').label = "EDITER LES SIG";
		}
		else {
			document.getElementById('BoutonEdition').label = "ENVOYER LES SIG";
		}
		...
		if (document.getElementById('TypeD').selected && !isDate(date_fin)) {
			...
			if (sortie=="M") {
				...
				if (!isEmail(email)) {
					...
					var corps = cookie() +"&Page=Compta/Etats/sig_pdf.tmpl&ContentType=xml&EnvoiMail=y";
          ...
          requeteHTTP(corps,new XMLHttpRequest(),suiteEditer);
				...
				page = getUrlOpeneas("&Page=Compta/Etats/sig_pdf.tmpl" + liste_options);
				document.getElementById('sig').setAttribute("src", page);
...
function suiteEditer(httpRequest) {
  try {
    ...
    showMessage("Les SIG ont été envoyés");
