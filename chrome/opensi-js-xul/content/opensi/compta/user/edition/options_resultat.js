...
function cacheChampsMail(b) {
	try {
		...
		if (b) {
			document.getElementById('BoutonEdition').label = "EDITER LE COMPTE DE RESULTAT";
		}
		else {
			document.getElementById('BoutonEdition').label = "ENVOYER LE COMPTE DE RESULTAT";
		}
		...
		if (document.getElementById('TypeD').selected && !isDate(date_fin)) {
			...
			if (sortie=="M") {
				...
				if (!isEmail(email)) {
					...
					var corps = cookie() +"&Page=Compta/Etats/resultat_pdf.tmpl&ContentType=xml&EnvoiMail=y";
          ...
          requeteHTTP(corps,new XMLHttpRequest(),suiteEditer);
				...
				page = getUrlOpeneas("&Page=Compta/Etats/resultat_pdf.tmpl" + liste_options);
				document.getElementById('resultat').setAttribute("src", page);
...
function suiteEditer(httpRequest) {
  try {
    ...
    showMessage("Le résultat a été envoyé");
